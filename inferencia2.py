#ejecutarse: python predict_binding_site.py mi_proteina.pdb modelo_rf_predictor.pkl
...
Outputs:
    ...
    - visualization.pml             → script para PyMOL
    - visualization.cxc             → script para ChimeraX
"""
...
# ─────────────────────────────────────────────
# 5. EXPORTAR SCRIPT CHIMERAX (.cxc)
# ─────────────────────────────────────────────

def guardar_chimerax(residuos, pdbfile, output="visualization.cxc"):
    """
    Genera un script .cxc que:
    - Abre el PDB
    - Colorea la proteína en gris
    - Selecciona y colorea el binding site en rojo
    - Muestra los residuos como sticks con superficie semitransparente
    """
    pdb_path = os.path.abspath(pdbfile)

    # Construir especificación de residuos para ChimeraX
    # Formato: /A:10,15,22 /B:5,8
    spec_por_cadena = {}
    for chain, res_id, _ in residuos:
        spec_por_cadena.setdefault(chain, []).append(str(res_id))

    spec_parts = []
    for chain, ids in spec_por_cadena.items():
        ids_str = ",".join(ids)
        spec_parts.append(f"/{chain}:{ids_str}")
    spec_string = " ".join(spec_parts)

    with open(output, 'w') as f:
        f.write(f"# ChimeraX script — Binding site prediction\n")
        f.write(f"# Generated by predict_binding_site.py\n\n")
        f.write(f"open {pdb_path}\n\n")
        f.write(f"# Representación base\n")
        f.write(f"hide\n")
        f.write(f"show cartoons\n")
        f.write(f"color gray\n\n")
        f.write(f"# Binding site\n")
        f.write(f"select {spec_string}\n")
        f.write(f"show sel atoms\n")
        f.write(f"style sel stick\n")
        f.write(f"color sel red\n\n")
        f.write(f"# Superficie semitransparente del binding site\n")
        f.write(f"surface sel\n")
        f.write(f"transparency 50 sel\n")
        f.write(f"color sel surface red\n\n")
        f.write(f"# Centrar la vista en el binding site\n")
        f.write(f"view sel\n")
        f.write(f"select clear\n")

    print(f"Script ChimeraX guardado en: {output}")
...
def predecir_binding_site(pdbfile, modelo_path):
    ...
    guardar_chimerax(residuos, pdbfile,
                     output=f"{pdb_base}_visualization.cxc")

    print("\n¡Predicción completada!")
    print(f"Archivos generados:")
    print(f"  - {pdb_base}_binding_site_residues.txt")
    print(f"  - {pdb_base}_visualization.pml")
    print(f"  - {pdb_base}_visualization.cxc")
