...
def mapear_residuos(sas_points, predicciones, lista_atomos, tree, radio=5.0):
    """
...
def predecir_binding_site(pdbfile):
    ...
    # Cargar modelo
    print("\nCargando modelo: modelo_rf_predictor500.joblib")
    modelo = joblib.load("modelo_rf_predictor500.joblib")
    ...
    # threshold = np.percentile(probabilidades, 87)  # top 13%
    # predicciones = (probabilidades >= 0.7).astype(int)
...
# ─────────────────────────────
    ...
    print("\nAplicando selección inicial (Top-K 8%)...")

    top_k = int(len(probabilidades) * 0.08)
    indices_top = np.argsort(probabilidades)[-top_k:]

    predicciones = np.zeros(len(probabilidades))
    predicciones[indices_top] = 1
...
# ─────────────────────────────
    ...
    print(f"Puntos iniciales (Top-K): {len(positivos)}")
...
# ─────────────────────────────
    if len(positivos) > 0:
        ...
        clustering = DBSCAN(eps=2.5, min_samples=15).fit(coords)
    ...
    # quitar ruido
        ...
        print(f"Tras quitar ruido: {len(positivos)}")

    # filtrar clusters pequeños
        cluster_sizes = positivos.groupby('cluster').size()
        clusters_grandes = cluster_sizes[cluster_sizes > 40].index
        positivos = positivos[positivos['cluster'].isin(clusters_grandes)]

        print(f"Clusters grandes detectados: {len(clusters_grandes)}")
        print(f"Tras filtrar clusters: {len(positivos)}")

    # filtro de densidad (suave)
        if len(positivos) > 0:
            coords = positivos[['x','y','z']].values
            tree_pos = KDTree(coords)

            densidades = [
                len(tree_pos.query_ball_point(p, 6.0))
                for p in coords
            ]

            positivos = positivos[np.array(densidades) > 4]
            print(f"Tras filtrar densidad: {len(positivos)}")
    ...
    # máscara cluster
        mask_cluster = np.zeros(len(predicciones))
        indices_validos = positivos.index.to_numpy()
        mask_cluster[indices_validos] = 1

    # puntos MUY seguros (rescate)
        high_conf = probabilidades >= np.percentile(probabilidades, 95)

    # combinación FINAL
        pred_filtrado = np.zeros(len(predicciones))

    # clusters buenos
        pred_filtrado[mask_cluster == 1] = 1

    # rescatar puntos muy seguros
        pred_filtrado[high_conf] = 1
